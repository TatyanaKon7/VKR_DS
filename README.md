...
С целью получения первоначальных представлений о характерах распределений переменных исходного набора данных, формирование оценки качества исходных данных (наличие пропусков, выбросов), выяв-ление характера взаимосвязи между переменными проводим разведочный анализ данных.
...
При анализе полученного датасета видим неинформативность пер-вого столбца, принимаем решение о его удалении. Размер полученного датасета – 1023 строки, 13 столбцов.
 
Рисунок 3 – Объединенный датасет без первого столбца
...
Данный датасет содержит переменные значения float64, качествен-ных характеристик нет, пропуски отсутствуют, ни одна из записей не яв-ляется NaN, очистка не требуется.
...
Рисунок 6 – Пропуски и дубликаты

В данном датасете отсутствуют пропуски и дубликаты, нулевых значений нет, следовательно, очистка не требуется.
При проверке на уникальность значений получаем, что столбец «Угол нашивки, град» имеет только два значения.
 
Рисунок 7 – Проверка уникальности значений

4.	среднее и медианное значение:
 
Рисунок 8 – Средние значения столбцов

 
Рисунок 9 – Медианное значение столбцов

При анализе полученных результатов видим значения, близкие друг к другу. Только столбец «Угол нашивки, град» имеет сильно отличающиеся значения.
5.	гистограммы распределения каждой из переменной:
 
Рисунок 10 – Гистограммы распределения каждой переменной

При проведении анализа получаем, что данные преимущественно стремятся к нормальному распределению. Угол нашивки, как и отражено в датасете, имеет только два значения. Учитывая отсутствие иных показателей для угла нашивки, в прогнозе будем использовать не непрерывный, а категориальный подход при анализе данного параметра.
6.	диаграммы «ящика с усами», анализ выбросов:
 
Рисунок 11 – «Ящики с усами»

Данные диаграммы показывают наличие выбросов данных во всех столбцах кроме столбца «Угол нашивки». Следовательно, в дальнейшем необходимо поработать с данными столбцов и исключить выбросы.
7.	попарные графики рассеяния точек относительно целевых переменных:

 
Рисунок 12 – Попарные графики рассеяния точек относительно пе-ременной «Модуль упругости при растяжении»

 
Рисунок 13 – Попарные графики рассеяния точек относительно пе-ременной «Прочность при растяжении»

Данные графики не показывают какой-либо линейной зависимости между данными. Кроме того, можно наблюдать выбросы, т.к. некоторые точки располагаются далеко от общего облака. Можно сделать предположение об отсутствии линейной корреляции.
8.	тепловая карта корреляционной зависимости:
 
Рисунок 14 – Тепловая карта корреляционной зависимости

Проводя анализ тепловой карты, видим максимальное значение корреляции между «Плотностью нашивки» и «Углом нашивки» равно 0,11. Все остальные значения близки к 0. Можем сделать вывод об отсутствии корреляционной зависимости между данными.
Мы получили первоначальных представлений о характерах распре-делений переменных исходного набора данных, формирование оценки качества исходных данных: отсутствие пропусков, дубликатов, наличие выбросов в каждом столбце кроме столбца «Угол нашивки, град»; сделали вывод об отсутствии явной корреляционной зависимости между данными. На данном этапе мы предполагаем, что зависимость между данными все-таки есть. Выбираем подходящие методы решения –  построение и обучение модели методом «Случайный лес», «К-ближайших соседей», «Дерево решений».
...
Рисунок 15 – Измененный датасет

Вновь смотрим среднее и медианное значения переменных. Убеждаемся в их незначительном отличии.
 
Рисунок 15 – Среднее и медианное значения

На этапе разведочного анализа, мы убедились, что в датасете при-сутствуют выбросы. Посчитаем их количество методом межквартильных расстояний (значения, удаленные более чем на 1,5 межквартильных рас-стояния, обнулим:
 
Рисунок 16 – Количество выбросов
...
Рисунок 17 – Удаление выбросов
Визуализируем наши данные. Убеждаемся в их разбросе и в отсут-ствии корреляции. Следовательно, нужно данные отмасштабировать.
 
Рисунок 18 – Распределение данных

Масштабирование проводим следующими методами: MinMaxScaler, Normalizer, StandardScaler.
 
Рисунок 19 – Тепловые карты отмасштабированных датасетов

Наибольшая зависимость между данными в датасете после Normalizer. Поэтому именно с ним проводим дальнейшую работу.
2.2.	Разработка и обучение модели
Для разработки моделей выбрано 3 метода: RandomForest, K-Nearest Neighbors, Decision Trees. Так как у нас 2 целевые переменные, они не должны участвовать в разработке и обучении моделей друг друга.
Отдельно создаем модели для переменной «Модуль упругости при растяжении» и «Прочность при растяжении».
 
Рисунок 20 – Создание моделей
2.3.	Тестирование модели
Тестируем модели, визуализируем результат.
 
Рисунок 21 – Тестовые и прогнозные значения переменной «Модуль упругости при растяжении» для разных моделей
 
Рисунок 22 – Тестовые и прогнозные значения переменной «Проч-ность при растяжении» для разных моделей
...
Рисунок 24 – Ошибки моделей для переменной «Модуль упругости при растяжении» до и после подбора гиперпараметров

 
Рисунок 25 – Ошибки моделей для переменной «Прочность при растяжении» до и после подбора гиперпараметров

Средняя квадратичная ошибка стремится к нулю, коэффициент де-терминации – к единице. Следовательно, можно говорить наших моделей.
2.4.	Создание нейронной сети
Создаем нейронную сеть с помощью Sequential.
 
Рисунок 26 – Создание нейронной сети
По значениям ошибок можно сказать, что нейронная сеть не-успешна.
2.5.	Разработка приложения
 
Рисунок 27 – Приложение
2.6.	Создание удаленного репозитория и загрузка результатов работы на него
 
Рисунок 28 – Удаленный репозиторий
...
2.1.	Изучим описательную статистику каждой переменной – среднее, медиана, стандартное отклонение, минимум, максимум, квартили.
2.2.	Проверим датасет на пропуски и дубликаты данных
3.	Визуализируем наш разведочный анализ данных (до выбросов и нормализации).
3.1.	Построим гистограмм распределения переменных.
3.2.	Построим диаграммы ящиков с усами каждой переменной.
3.3.	Построим попарные графики рассеяния точек относительно целевых переменных.
3.4.	Построим корреляционную матрицу с помощью тепловой карты.
3.5.	Сохраняем датасет.
4.	Проведём предобработку данных.
4.1.	Меняем название столбца «Угол нашивки, град» на «Угол нашивки» и его значения: 0 на 0, 90 – 1.
4.2.	Проверим выбросы методом межквартильных расстояний. 
4.3.	Посчитаем распределение выбросов по каждому столбцу.
4.4.	Создаем переменную с названиями столбцов за исключением целевых переменных.
4.5.	Исключим выбросы методом межквартильных расстояний (обнулим значения выбросов).
4.6.	Удалим строки c выбросами.
4.7.	Для полной очистки датасета от выбросов повторим пункты (4.5 – 4.6) ещё 3 раза. 
4.8.	Изучим чистые данные по всем параметрам.
4.9.	Визуализируем датасет без выбросов.
5.	Проведём нормализацию и стандартизацию (продолжим предобработку данных).
5.1.	Нормализуем данные с помощью MinMaxScaler().
5.2.	Нормализуем данные с помощью Normalizer().
5.3.	Визуализируем данные.
5.4.	Сравним с данными до нормализации.
5.5.	Строим тепловую карту корреляции.
5.6.	Стандартизируем данные
5.7.	Визуализируем данные корреляции
5.8.	Посмотрим на описательную статистику после нормализации и после стандартизации
6.	Разработаем и обучим нескольких моделей прогноза модуля упругости при растяжении (с 30% тестовой выборки)
...
6.5.	Построим и визуализируем результат работы метода К ближайших соседей.
6.6.	Построим и визуализируем результат работы метода деревья решений.
6.7.	Сравним наши модели по метрикам.
6.8.	Найдём лучшие гиперпараметры для случайного леса.
6.9.	Подставим значения в нашу модель случайного леса.
6.10.	Найдём лучшие гиперпараметры для К ближайших соседей.
6.11.	Подставим значения в нашу модель К ближайших соседей.
6.12.	Найдём лучшие гиперпараметры метода деревья решений.
6.13.	Подставим значения в нашу модель метода деревья решений.
6.14.	Сравним все модели и процессинги и выведем лучшую модель и процессинг.
...
7.4.	Построим и визуализируем результат работы метода случайного леса.
7.5.	Построим и визуализируем результат работы метода К ближайших соседей.
7.6.	Построим и визуализируем результат работы метода деревья решений.
7.7.	Сравним наши модели по метрикам.
7.8.	Найдём лучшие гиперпараметры для случайного леса.
7.9.	Подставим значения в нашу модель случайного леса.
7.10.	Найдём лучшие гиперпараметры для К ближайших соседей.
7.11.	Подставим значения в нашу модель К ближайших соседей.
7.12.	Найдём лучшие гиперпараметры метода деревья решений.
7.13.	Подставим значения в нашу модель метода деревья решений.
7.14.	Сравним все модели и процессинги и выведем лучшую модель и процессинг.
...
8.4.	Посмотрим на результаты.
8.5.	Обучим нейросеть 70/30.
8.6.	Оценим модель.
8.7.	Посмотрим на график потерь на тренировочной и тестовой выборках.
8.8.	Сконфигурируем другую модель, зададим слои.
8.9.	Посмотрим на архитектуру другой модели.
8.10.	Обучим другую модель.
8.11.	Оценим модель.
8.12.	Посмотрим на график потерь на тренировочной и тестовой выборках.
8.13.	Сохраняем вторую модель для разработки веб-приложения для прогнозирования соотношения «матрица-наполнитель» в фреймворке Flask.
9.	Создаём приложение.
9.1.	Импортируем необходимые бибилиотеки.
...
10.1.	https://github.com/TatyanaKon7
10.2.	Создадим README (https://github.com/TatyanaKon7/VKR_DS/blob/main/README.md)
